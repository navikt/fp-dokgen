package no.nav.foreldrepenger.dokgen.tjenester.pdf;

import static no.nav.foreldrepenger.dokgen.tjenester.utils.ContentUtil.lesRessursFra;
...
public class PdfGeneratorTjeneste {
    ...
    private PDFontSupplier fontSupplier(String fontName) {
        var font = FONT_CACHE.computeIfAbsent(fontName, name -> {
            try {
                var ttf = new TTFParser().parse(
                    new RandomAccessReadBuffer(lesRessursFra(ContentUtil.hentFontDirectoryPath().resolve(name))
                    )
                );
